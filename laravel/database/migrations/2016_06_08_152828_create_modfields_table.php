...
class CreateModfieldsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('modfields', function (Blueprint $table) {
            ...
            $table->integer('f_mod_id')->length(2)->comment('所属模型id');
            ...
            $table->integer('f_is_index')->length(1)->default(0)->comment('是否加索引,1为加索引,0为不加索引');
            $table->integer('f_is_unique')->length(1)->default(0)->comment('字段值是否唯一,1为唯一,0为不唯一');
            $table->integer('f_is_sys')->length(1)->default(0)->comment('是否系统字段,1为系统字段,0为自己增加的字段');
            $table->integer('f_is_intro')->length(1)->default(0)->comment('是否简介字段,1为简介字段,0为普通字段');
            $table->integer('f_is_subtb')->length(1)->default(1)->comment('是否为副表字段,0为主表,1为副表');
            ...
            $table->integer('f_html_support')->length(1)->default(0)->comment('内容支持html代码,1为支持,0为不支持,程序处理时要过滤标签');
