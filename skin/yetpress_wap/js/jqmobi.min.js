if(!window.jm||typeof(jm)!=="function"){
	...
	function androidVersion(){
		...
		detectUA(jm,navigator.userAgent);
	jm.__detectUA=detectUA;if(typeof String.prototype.trim!=="function"){String.prototype.trim=function(){this.replace(/(\r\n|\n|\r)/gm,"").replace(/^\s+|\s+$/,"");return this}}
	jm.getCookie=function(c_name){if(document.cookie.length>0){c_start=document.cookie.indexOf(c_name+"=");if(c_start!=-1){c_start=c_start+c_name.length+1;c_end=document.cookie.indexOf(";",c_start);if(c_end==-1){c_end=document.cookie.length}return decodeURIComponent(document.cookie.substring(c_start,c_end))}}return""};
	jm.setCookie=function(c_name,value,expiredays){var exdate=new Date();exdate.setDate(exdate.getDate()+expiredays);document.cookie=c_name+"="+encodeURIComponent(value)+((expiredays==null)?"":";expires="+exdate.toGMTString())};
	...
	jm.uuid=function(){var S4=function(){return(((1+Math.random())*65536)|0).toString(16).substring(1)};return(S4()+S4()+"-"+S4()+"-"+S4()+"-"+S4()+"-"+S4()+S4()+S4())};
	jm.getCssMatrix=function(ele){if(ele==undefined){return window.WebKitCSSMatrix||window.MSCSSMatrix||{a:0,b:0,c:0,d:0,e:0,f:0}}try{if(window.WebKitCSSMatrix){return new WebKitCSSMatrix(window.getComputedStyle(ele).webkitTransform)}else{if(window.MSCSSMatrix){return new MSCSSMatrix(window.getComputedStyle(ele).transform)}else{var mat=window.getComputedStyle(ele)[jm.feat.cssPrefix+"Transform"].replace(/[^0-9\-.,]/g,"").split(",");return{a:+mat[0],b:+mat[1],c:+mat[2],d:+mat[3],e:+mat[4],f:+mat[5]}}}}catch(e){return{a:0,b:0,c:0,d:0,e:0,f:0}}};var handlers={},_jqmid=1;function jqmid(element){return element._jqmid||(element._jqmid=_jqmid++)}
	function findHandlers(element,event,fn,selector){event=parse(event);if(event.ns){var matcher=matcherFor(event.ns)}return(handlers[jqmid(element)]||[]).filter(function(handler){return handler&&(!event.e||handler.e==event.e)&&(!event.ns||matcher.test(handler.ns))&&(!fn||handler.fn==fn||(typeof handler.fn==="function"&&typeof fn==="function"&&""+handler.fn===""+fn))&&(!selector||handler.sel==selector)})}
