...
?>  <tr bgcolor="#FFFFFF"> 
  ...
  <tr bgcolor="#FFFFFF"> 
    <td height="22" valign="top"><strong>视频附件正则：</strong><br>
      ( 
      <input name="textfield" type="text" id="textfield" value="[!--video_file--]" size="20">
      )</td>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="3">
    <tr>
      <td>附件前缀 
        <input name="add[qz_video_file]" type="text" id="add[qz_video_file]" value="<?=stripSlashes($r[qz_video_file])?>"> 
        <input name="add[save_video_file]" type="checkbox" id="add[save_video_file]" value=" checked"<?=$r[save_video_file]?>>
        远程保存 </td>
    </tr>
    <tr> 
      <td><textarea name="add[zz_video_file]" cols="60" rows="10" id="add[zz_video_file]"><?=ehtmlspecialchars(stripSlashes($r[zz_video_file]))?></textarea></td>
    </tr>
    <tr> 
      <td><input name="add[z_video_file]" type="text" id="video_file5" value="<?=stripSlashes($r[z_video_file])?>">
        (如填写这里，这就是字段的值)</td>
    </tr>
  </table></td>
  </tr>
