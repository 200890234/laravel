...
//-------- 自定义分页样式
function yet_wap_pager($num,$line,$page,$search){
	if(empty($num))
	{
		return '';
	}
	...
	{
		if($page==1){//第一页
			$str.='<div class="pagePrev icon-pagePrev icon-cnListP pageDisable">上一页</div>';
		}else{
			$str.='<div class="pagePrev icon-pagePrev icon-cnListP "><a href="'.$phpself.'?page='.($page-1).$search.'">上一页</a></div>';
		}
	}
	if($page)
	{	
		$options="";
		for($go=1;$go<=$pagenum;$go++){//分页下拉
			if($page==$go){
				$select=" selected";
			}else{
				$select="";
			}
			$jumpurl=$phpself.'?page='.$go.$search;
			$options.="<option class='pageOptions' value='".$jumpurl."'".$select.">".$go.'&nbsp;/&nbsp;'.$pagenum."</option>";
		}
		...
		$str.='<div id="pagenationSelectDiv3" class="pagenationSelectDiv">'
				.$options.
			'</div>';
	}
	if($page)
	{
		if($page==$pagenum){//已经是最后一页
			$str.='<div class="pageNext icon-pageNext icon-cnListN pageDisable">下一页</div>';
		}else{
			$str.='<div class="pageNext icon-pageNext icon-cnListN "><a href="'.$phpself.'?page='.($page+1).$search.'">下一页</a></div>';
		}
	}
	return $str;
}
